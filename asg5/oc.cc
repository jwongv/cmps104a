...
int main(int argc, char **argv){
   ...
   for(int argi = optind; argi < argc; ++argi){
      ...
      strcpy(output, basename(filename));
         ...
         strcpy(symoutput, output);
         strcpy(oiloutput, output);
         strcat(stroutput, ".str");
         strcat(tokoutput, ".tok");
         strcat(astoutput, ".ast");
         strcat(symoutput, ".sym");
         strcat(oiloutput, ".oil");
      string command;
      ...
      //START TO MAKE SYM TABLES
      FILE* symoutputfile = fopen(symoutput, "w");
      SymbolTable* global_sym = new SymbolTable(NULL);
//      SymbolTable* struc_sym = new SymbolTable(NULL);
      scan_tree(yyparse_astree, global_sym);
      global_sym->dump(symoutputfile,0);
      fclose(symoutputfile);
      type_check(yyparse_astree, global_sym);
//      string rawrrr = "hello";
//      rawrrr = rawrrr + ';';
//      printf("%s\n", rawrrr.c_str());
      ...
      oiloutputfile = fopen(oiloutput, "w");
      fprintf(oiloutputfile, "#define __OCLIB_C__\n#include \"oclib.oh\"\n");
      code_gen(yyparse_astree, global_sym);
